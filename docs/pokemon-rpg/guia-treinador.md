...
- **⬇️ Exportar CSV** — baixa a Equipe atual (até 6 Pokémon) numa planilha, com Espécie/Natureza/Habilidade/Atributos Naturais/Vínculo/Maturidade/Movimentos de cada um. Qualquer pessoa que vê a ficha pode exportar (é só leitura) — útil pra guardar um backup antes de o Mestre importar algo novo.
- **⬆️ Importar CSV** — só o **Mestre** vê esse botão (é ele quem define Espécie/Natureza/Habilidade/Atributos, então é ele quem importa). Escolher um arquivo mostra um resumo (quantos Pokémon têm no arquivo, quantos existem hoje, e qualquer aviso — Espécie que não bateu com nenhuma da Pokédex, Movimento inválido pra aquela Espécie/Nível etc.) antes de confirmar. **Importar SUBSTITUI a Equipe inteira** pelo conteúdo do arquivo — por isso vale exportar a Equipe atual primeiro, se quiser guardar o que já estava lá.
...
- **⬇️ Exportar CSV / ⬆️ Importar CSV / 📄 Modelo** (abaixo das fichas): igual à Equipe do RPG (ver "Exportar/Importar CSV da Equipe" acima), mas totalmente **independente** — importar aqui nunca mexe no seu Time do RPG, e vice-versa. Como as Fichas de PVP não têm Vínculo/Maturidade/Atributo Natural (tudo já fixo no máximo), essas colunas nem aparecem nessa planilha. Diferente da Equipe (Mestre-only), aqui **você mesmo** pode importar — é a sua própria ficha de PVP. Fica bloqueado se alguma das suas fichas estiver numa batalha em andamento (espere terminar).
