...
### Iniciativa (seleção simultânea + resolução automática)
...
Assim que todo mundo escolheu, a Rodada resolve **sozinha**: o site processa um Pokémon de cada vez, com um pequeno espaçamento entre cada um, nesta ordem — **não-Ataque (Trocar/Item/Fugir) sempre antes de qualquer Ataque, mesmo um Ataque de Prioridade**; entre Ataques, **Prioridade decide antes da Velocidade** (Cap. 16/17: Velocidade Efetiva, depois Natural, depois 1d6 em empate). Um Pokémon Paralisado entra no cálculo com −10 de Velocidade, igual à regra. **Não existe mais nenhuma pergunta de "quer usar sua Prioridade?"** — quem tinha um Movimento de Prioridade já decidiu usá-lo (ou não) na hora de escolher, junto com o resto da mesa, e a Prioridade simplesmente entra na frente sozinha. Assim que a Rodada inteira termina de resolver, a próxima já abre pra seleção sozinha, sem o Mestre precisar clicar em nada de novo — isso se repete a batalha inteira.
...
3. **A Rodada resolve** assim que todo mundo escolheu (ou o Mestre força "Resolver Rodada"): o site processa cada ação na ordem certa (ver "Iniciativa" acima) — é só nesse momento que cada coisa de fato acontece:
