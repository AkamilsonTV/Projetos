...
Não existe mais botão de "Usar" nem de "Atacar" separado — é tudo clicando **direto no card do seu Pokémon** em campo (contanto que ele não esteja Desmaiado e não seja a vez de outra ação já em andamento). Clicar já abre um **popup** — não é mais um card fixo lá embaixo da página, então não precisa rolar a tela pra ver ele nem pra escolher o alvo: o seu Pokémon fica sempre visível no topo do popup, e assim que você escolhe um Movimento (ou Item) que precise de um alvo manual, quem pode ser alvo já aparece logo abaixo, dentro do mesmo popup — clique ali mesmo. Trocar de Pokémon funciona igual: os candidatos da Equipe ativa aparecem dentro do popup também. O fluxo é:
...
2. Se a ação seguir, abre um menu com quatro opções: **Atacar**, **Usar Item**, **Trocar de Pokémon**, **Fugir**.
   - **Atacar** → escolha um dos Movimentos do seu Pokémon — cada um já mostra **quem pode ser alvo** dele ("Alvo: em você mesmo", "só aliado", "aliado ou inimigo" etc.) e a Prioridade, se tiver. Sem PP, o Movimento fica bloqueado (custa 1, ou 2 se o alvo tiver a Habilidade Pressure — Cap. 8.6) — some sozinho assim que o Movimento é usado de verdade. A lógica de cada Movimento é respeitada de verdade — o site não deixa escolher qualquer coisa:
     ...
     - Pra qualquer outro caso (**só aliado**, **você ou aliado**, **só inimigo**, ou **aliado ou inimigo**), o popup já mostra logo abaixo do seu Pokémon quem pode ser alvo — clique num deles ali dentro pra resolver; clicar em alguém fora da lista não faz nada.
   ...
   - **Usar Item** → a mochila aparece separada por tipo (💊 Vida, ✨ Condição, 💫 Reviver, 🔄 Vida + Condição, 🔋 PP, 📦 Outro), igual na Mochila da ficha — Pokébola não aparece aqui também, pelo mesmo motivo (sem sistema de captura ainda). Escolha o item e depois em quem usar — a lista de alvos elegíveis (você mesmo ou um aliado em campo) já aparece dentro do popup, respeitando a trava de desmaiado (Vida/Vida+Condição/PP só em quem está de pé, Reviver só em quem está desmaiado). Pra um item de PP que afeta um Movimento só (Ether, Max Ether, PP Up, PP Max), o popup ainda pede pra escolher qual dos 4 Movimentos recebe. Itens de Vida, Condição, Reviver, Vida+Condição e PP já fazem efeito de verdade na hora — PV sobe, Condição/Confusão some, o Pokémon reanima e/ou o PP do Movimento aumenta — e o Log mostra o resultado. Itens de Outro tipo ainda não têm efeito mecânico (Cap. 25): só consomem 1 unidade e ficam registrados.
