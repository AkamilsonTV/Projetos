...
Logo acima da Equipe/Banco, cada ficha tem a seção **"🎒 Mochila"** — os itens organizados em **cinco abas** (cada uma como uma tabela Nome/Descrição/Quantidade): **💊 Poções e Berries**, **⚪ Pokébolas**, **🗝️ Itens de Missão**, **💿 TM** e **🎒 Inventário Geral** (qualquer item que não caiba nas quatro anteriores). **Só você adiciona, remove e muda a quantidade** de um item — o Treinador só vê a tabela (nome, descrição, quantidade), sem poder editar. Pra adicionar, preencha:

- **Nome**: tem autocomplete com o catálogo oficial (ver "Categorias de Itens" abaixo, ~415 itens de todo o jogo, incluindo as 229 TMs vigentes de Scarlet/Violet). Escolher um nome da lista **preenche Descrição, Tipo, PV, Condição curada, Reviver total, os campos de PP e Categoria sozinho** — já convertidos pro nosso sistema — você pode continuar editando à mão depois se quiser fugir do oficial. Digitar um nome que não bate com nada do catálogo funciona normal, só fica sem categoria (cai na aba Inventário Geral).
- **Descrição**: preenchida sozinha com um item do catálogo (a mesma descrição oficial, em inglês, igual os Movimentos); pra um item homebrew, escreva a sua.
- **Quantidade**.
...
- **Alvo de PP / Enche até o máximo / Aumenta o Máximo**: só aparecem pro tipo PP (Cap. 8.6.1) — "Alvo" escolhe se o item afeta **um** Movimento (Ether, PP Up, PP Max) ou **todos** os 4 de uma vez (Elixir, Max Elixir); "Enche até o máximo" marca se enche o PP Atual de uma vez (Max Ether/Max Elixir) ou só soma uma quantidade fixa (Ether/Elixir, +10); "Aumenta o Máximo" marca se o item também aumenta o PP Máximo do Movimento, não só recupera o Atual (PP Up/PP Max).
...
No seu Painel do Mestre ("Mesa"), logo abaixo da contagem de Treinadores, tem o card **"🎒 Categorias de Itens"**. Ele lista as 11 categorias do catálogo oficial — Poções, Curas de Status, Revives, Pokébolas, Itens de Batalha, Itens de Evolução, Berries, Itens Segurados, Itens-Chave, Recuperação de PP e TMs — cada uma com um interruptor. **Desligar uma categoria some ela do autocomplete de nome ao adicionar item pra mesa inteira** (os itens que já foram adicionados na Mochila de alguém continuam lá normalmente, só some da sugestão pra novos itens). Use isso pra simplificar a mesa — por exemplo, desligar Pokébolas e Itens-Chave se sua campanha não usa captura.

Poções, Curas de Status, Revives, as Berries que curam PV/Condição e Recuperação de PP têm efeito mecânico de verdade hoje (Cap. 25/8.6.1); o resto (Pokébolas, Itens de Batalha, Itens de Evolução, Itens Segurados, Itens-Chave, TMs, e as Berries de batalha tipo Salac/Liechi) está catalogado — com descrição oficial — mas ainda sem regra própria: fica registrado na Mochila, sem efeito automático.

### Cura (fora de batalha)

Cada ficha (incluindo pela sua visão de Mestre) também tem o botão **"🩹 Cura"**, logo abaixo da Mochila — abre as abas 💊 Vida / ✨ Condição / 💫 Reviver / 🔄 Vida + Condição / 🔋 PP pra usar um item de cura num Pokémon daquele Treinador sem precisar estar na Arena. Funciona igual pro Treinador usar na própria ficha: escolhe a aba, o item, o Pokémon (a lista já respeita a trava de desmaiado — Reviver só mostra quem está desmaiado, as outras quatro só mostram quem não está) e, só pros itens de PP que afetam um Movimento só (Ether/Max Ether/PP Up/PP Max), qual dos 4 Movimentos recebe. A cura acontece na hora (PV sobe, Condição/Confusão some, o Pokémon reanima e/ou o PP do Movimento aumenta), consumindo 1 unidade. Itens tipo Outro não aparecem aqui — ainda não têm efeito (Cap. 25).
