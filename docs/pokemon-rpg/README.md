...
- **PP (Cap. 8.6)**: `MOVES_DEX` ganhou o campo `pp` (PP oficial de cada um dos 847 Movimentos, via PokeAPI). Cada slot de Movimento do Pokémon tem `pp` (Máximo) e `ppAtual` — `applyMoveData` preenche os dois (Atual = Máximo) sempre que um Movimento novo é escolhido/digitado, tanto pelo Mestre (`DT.setMove`) quanto pelo Treinador via Learnset (`DT.setMoveSelect`); `migratePokemonMoves` faz o mesmo backfill (idempotente) em fichas salvas de antes desse campo existir. `DT.setMovePP` edita o Atual (clamped 0..Máximo), disponível tanto pro Mestre quanto pro dono do Pokémon, igual PV Atual; `DT.setMove` com `field:'pp'` deixa o Mestre editar o Máximo à mão (clamped 0..40), puxando o Atual pra baixo se ele passar do novo teto. Na Arena, `DT.arenaPickMove` recusa escolher um Movimento com `ppAtual<=0` quando `pp>0` (Movimento sem PP cadastrado, ex. homebrew que não bateu com a base, nunca bloqueia); `DT.resolveArenaMove` desconta 1 PP, ou 2 se qualquer alvo resolvido (incluindo o próprio usuário, num Movimento `self`) tiver a Habilidade Pressure; `DT.resolveFieldMove` também desconta 1. Sem Struggle automático — 0 PP nos 4 Movimentos é decisão do Mestre.
