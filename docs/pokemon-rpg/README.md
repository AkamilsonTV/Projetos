...
- **Habilidades** (`ABILITIES_DEX`, 373 entradas reais `{name,efeito}` via PokeAPI; `SPECIES_DEX` ganhou o campo `hab:[{n,h}]` por espécie, `h`=Habilidade Oculta): cada Pokémon tem `habilidade` (Mestre só, `DT.setHabilidade`) restrita às opções de `habilidadesDaEspecie(p.especie)` — trocar a Espécie (`DT.setPokeField`) limpa a Habilidade se ela não existir mais na nova espécie. Mostra o efeito oficial (`findAbilityByName`) como legenda, só de referência de mesa — sem gatilho automático em batalha, igual ao texto de Efeito dos Movimentos.
