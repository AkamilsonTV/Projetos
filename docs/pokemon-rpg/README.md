...
- **Mochila (itens) com tipo e cura de verdade**: cada Treinador tem uma lista de itens na própria ficha, gerenciada só pelo Mestre (`DT.addItem`/`removeItem`/`setItemQty`); aparece na Arena — cada um vê a própria, o Mestre vê a de todo mundo. Cada item tem um `tipo` (`'vida'`, `'condicao'` ou `'outro'`, `newItem`) — Vida cura `cura` PV (0 = cura total) e Condição cura `condicaoAlvo` (`''` = qualquer, tipo Full Heal; `'veneno'` cura Veneno e Veneno Grave), aplicado de verdade por `applyItemCure` (compartilhado entre a aba Cura da ficha e a Arena). No menu de ação, **Usar Item** já separa a lista por tipo e cura o alvo (o próprio Pokémon ou um aliado) na hora — itens `'outro'` continuam só descontando 1 unidade e registrando no log, sem efeito (Cap. 25).
- **Aba Cura na ficha** (`curaHTML`, fora de batalha): abre a Mochila filtrada em duas abas (Vida/Condição) — escolhe o item, escolhe o Pokémon da própria ficha (Equipe ou Banco), `DT.curaApply` aplica `applyItemCure` direto, sem precisar da Arena. Só o dono (`mode==='own'`) ou o Mestre (`mode==='gm'`) veem o botão.
...
Todo o núcleo de combate (atributos, Vínculo, PV, Movimentos, dano, tipos, iniciativa, Prioridade, condições de status — incluindo dano de status por turno e Confusão pré-ação automáticos na Arena), a progressão de Nível por batalhas vencidas, a Pokédex completa (Gen 1–9) e o banco de Movimentos oficiais, a organização de Equipe/Banco por arrastar, e uma Mochila de itens por Treinador (gerenciada pelo Mestre) — com cura de PV e de Condição/Confusão de verdade (itens tipo Vida/Condição, fora de batalha na aba Cura ou dentro da Arena). Ainda em aberto: Maturidade/Evolução definitiva, Captura, PP, efeito mecânico de qualquer item que não seja de cura, e Habilidades.
