...
- **Equipe ativa é pública**: `arenaSourceHTML()` renderiza o card "Equipe ativa" de **todos** os Treinadores (`STATE.trainers.map(arenaTrainerSourceHTML)`) pra qualquer viewer, jogador ou Mestre — sprite e nome de verdade de quem cada um tem, sem segredo nenhum aqui. O que continua restrito é o **controle**: `arenaTrainerSourceHTML` só renderiza o `drag-handle` (arrastar pra um Time) e o seletor de troca (`arena-source-tile-swappable`) quando `canControl` é verdadeiro (`SESSION.role==='gm'` ou o próprio dono) — ver outros Treinadores não dá permissão de mexer neles.
- **Visibilidade dentro de um Time**: o segredo começa só quando o Pokémon entra num Time. O dono e o Mestre sempre veem o sprite pixelado real, uma **barra de PV com três cores** (verde/amarela/vermelha, como no jogo oficial) e a Condição/Confuso como ícone (gráfico, não escrito — 🔥☠️⚡💤❄️💫😵). Os demais Treinadores veem uma Pokébola (o mesmo SVG do topo do site) até o Pokémon ser selecionado pra agir a primeira vez (revela sozinho nesse clique, sem botão separado pra isso), quando fica visível (e a barra de PV) pra mesa inteira. Essa revelação é **persistida no próprio Pokémon** (`p.arenaRevelado`, não na entrada da Arena) — uma vez revelado, continua com a sprite visível pra sempre, mesmo saindo de campo (Trocar) e entrando de novo depois com uma entrada nova.
